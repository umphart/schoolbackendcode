...
router.get('/api/students/primary', (req, res) => {
    const query = `SELECT * FROM primary_students`;
    db.query(query, (err, results) => {
      if (err) {
        console.error('Error fetching student list:', err);
        return res.status(500).send('Error fetching student list');
      }
      res.json(results);  // Return all students from primary section
    });
});

router.get('/api/students/junior', (req, res) => {
    const query = `SELECT * FROM junior_students`;
    db.query(query, (err, results) => {
      if (err) {
        return res.status(500).send('Error fetching student list');
      }
      res.json(results);  // Return all students from junior section
    });
});

router.get('/api/students/senior', (req, res) => {
    const query = `SELECT * FROM senior_students`;
    db.query(query, (err, results) => {
      if (err) {
        return res.status(500).send('Error fetching student list');
      }
      res.json(results);  // Return all students from senior section
    });
});
...
// Fetch Primary 1 students and count
router.get('/api/primary1', (req, res) => {
  const query = `SELECT * FROM primary_students WHERE class = 'Primary 1'`;
